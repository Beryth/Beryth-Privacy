...
function makeFakeDebugInfo(): Record<string, number> {
  const obj: Record<string, number> = {};
  Object.defineProperties(obj, {
    UNMASKED_VENDOR_WEBGL: {
      value: UNMASKED_VENDOR,
      writable: false,
      enumerable: true,
      configurable: false,
    },
    UNMASKED_RENDERER_WEBGL: {
      value: UNMASKED_RENDERER,
      writable: false,
      enumerable: true,
      configurable: false,
    },
...
export function applyWebglGuard(profile: Profile, mode: string): void {
  ...
  const rand = seededRandom(ORIGIN_SEED ^ 0x5745_4247); 
  ...
  const extCache = new WeakMap<object, object | null>();
  ...
  for (const proto of protos) {
    ...
    proto.getParameter = defineNative(function (
      ...
      if (pname === UNMASKED_VENDOR) {
        return block ? "" : spoofVendor;
      }
      if (pname === UNMASKED_RENDERER) {
        return block ? "" : spoofRenderer;
      }
      if (!block) {
    ...
    proto.getExtension = defineNative(function (
      ...
      if (name === "WEBGL_debug_renderer_info") {
        ...
        let cached = extCache.get(this);
        if (cached === undefined) {
          cached = makeFakeDebugInfo();
          extCache.set(this, cached);
        }
        return cached;
      }
    ...
    proto.getSupportedExtensions = defineNative(function (
      ...
      return exts.filter((e) => common.has(e)).sort();
